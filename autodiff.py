    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if(node not in node_to_val_map):
                ...
                out_val = node.op.compute(node, input_vals)
                node_to_val_map[node] = out_val
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        ...
        # sum of gradient list
        grad = grad_list[0]
        for i in range(1, len(grad_list)):
            grad = add_op(grad, grad_list[i])

        # gradient for input nodes
        input_nodes = node.inputs
        if len(input_nodes) > 0:
            grads = node.op.gradient(node, grad)
            assert len(input_nodes) == len(grads)
            for i in range(len(input_nodes)):
                if(input_nodes[i] in node_to_output_grads_list):
                    node_to_output_grads_list[input_nodes[i]].append(grads[i])
                else:
                    node_to_output_grads_list[input_nodes[i]] = [grads[i]]
